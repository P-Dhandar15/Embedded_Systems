...
// Select PIN Functionality--------------------------------------
 ...
 void PINSEL( port_Num, pin_Num, fun)
 {
	 int pin;
		if(port_Num == 0)
		{
			if(pin_Num >= 0 && pin_Num <= 15)
			{
				...
				if(fun == 1)
				{
						PINSEL0 = PINSEL0 | (0x01 << pin);
				}
				else if(fun == 2)
				{
						PINSEL0 = PINSEL0 | (0x10 << pin);
				}
				else if(fun == 3)
				{
						PINSEL0 = PINSEL0 | (0x11 << pin);
				}
			}
			
			else if(pin_Num >= 16 && pin_Num <= 31)
			{
				pin = 2*pin_Num;
				
				if(fun == 1)
				{
						PINSEL1 = PINSEL1 | (0x01 << pin);
				}
				else if(fun == 2)
				{
						PINSEL1 = PINSEL1 | (0x10 << pin);
				}
				else if(fun == 3)
				{
						PINSEL1 = PINSEL1 | (0x11 << pin);
				}
			}
		}
		...
		else if(port_Num == 1)
		{
			if(pin_Num >= 16 && pin_Num <= 31)
			{
				...
				if(fun == 1)
				{
						PINSEL2 = PINSEL2 | (0x01 << pin);
				}
				else if(fun == 2)
				{
						PINSEL2 = PINSEL2 | (0x10 << pin);
				}
				else if(fun == 3)
				{
						PINSEL2 = PINSEL2 | (0x11 << pin);
				}
			}
		}
  }